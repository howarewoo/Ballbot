...
void readIMU(){
  ...
  // Serial print and/or display at 0.005 s rate independent of data rates
  delt_t = millis() - count;
  if (delt_t > Ts*500) { // update LCD once per half-second independent of read rate
    printFlag = true;
...
void setupStepper(){  // Set a PWM signal to the step pins with 50% duty cycle
  ...
  analogWriteFrequency(STEP1, 0); // pins 2, 7, 8, 14, 35, 36, 37, 38 also change
  analogWriteFrequency(STEP2, 0); // pins 3, 4 also change
  analogWriteFrequency(STEP3, 0); // pins 5, 6, 9, 10, 20, 21, 22, 23 also change
}
...
void loop(){
  ...
  if (abs(180*(double)xAngle/PI) < .5 && abs(180*(double)yAngle/PI) < .5 && startMotors !=1){
    startMotors = 1;
  }
